...
static void periodicAdvProcess(void);
static float pressureBuffer[WINDOW_SIZE] = {0};
static uint8_t bufferIndex = 0; // Current index of the moving average buffer
...
// Set current alt detection led to LED_BLUE
...
void MX_BLESensorsPnPL_Process(void)
{
    ...
    {
      // Custom Code - Dante
      // Periodically start scanning every 10 seconds
      // if (HAL_GetTick() - scan_timer > 10000)
      // {
      //   StartBLEScan();
      //   HAL_Delay(5000); // Scan for 5 seconds
      //   StopBLEScan();
    ...
    // BSP_LED_Toggle(currentAltDetectionLed);
    // HAL_Delay(200);
    // BSP_LED_Off(currentAltDetectionLed);
    ...
    // Custom Code - Dante - Comment out this line for original code behaviour
    // SendEnv = 1;
    if (SendEnv) {
      float Temp,Pressure;
      SendEnv=0;
      ...
      // STBOX1_PRINTF("Custom data First Byte: 0x%02x\n", manuf_data[18]);
      // manuf_data[18] = 0x00;
      ...
      // if (pressureBuffer[bufferIndex] == 0) {
      //   runningSum += Pressure;
      //   initMovingAverage = 0;
      // } else {
      //   runningSum -= pressureBuffer[bufferIndex]; // Subtract the oldest value from the sum
      //   runningSum += Pressure; // Add the new value to the sum
      // }

      // pressureBuffer[bufferIndex] = Pressure; // Store the new value in the buffer
      // bufferIndex = (bufferIndex + 1) % WINDOW_SIZE; // Update the index in a circular manner

      // float movingAverage = runningSum / WINDOW_SIZE; // Calculate the moving average
      // // STBOX1_PRINTF("Current moving average:  \r\n");
      // // STBOX1_PRINTF("%.2f\r\n", movingAverage);
      // // manuf_data[BLE_MANAGER_CUSTOM_FIELD3]=0xFF;

      // if (initMovingAverage == 0) {
      //   initMovingAverage = movingAverage; // Store the initial moving average value
      ...
      //   if (movingAverage > (initMovingAverage + 0.05)) {
      //     // Fault has occurred
      ...
      //   } else if (movingAverage < (initMovingAverage - 0.05)) {
...
static void periodicAdvProcess(void)
{
  /* Function to send periodic BLE Advertisements */
...
//     {
//         STBOX1_PRINTF("BLE scanning started...\r\n");
//     }
//     else
//     {
//         STBOX1_PRINTF("Failed to start BLE scanning. Status: 0x%02X\r\n", status);
//     }
// }

// Callback to handle advertising reports
...
//         STBOX1_PRINTF("Device found:\r\n");
//         STBOX1_PRINTF("  RSSI: %d dBm\r\n", rssi);
//         STBOX1_PRINTF("  Advertising Data: ");
//         for (uint8_t j = 0; j < adv_data_len; j++)
//         {
//             STBOX1_PRINTF("%02X ", adv_data[j]);
//         }
//         STBOX1_PRINTF("\r\n");
//     }
// }
...
//     if (status == BLE_STATUS_SUCCESS)
//     {
//         STBOX1_PRINTF("BLE scanning stopped.\r\n");
//     }
//     else
//     {
//         STBOX1_PRINTF("Failed to stop BLE scanning. Status: 0x%02X\r\n", status);
//     }
// }
...
static void InitTimers(void)
{
  ...
  // Sets Env Sensor Measurement Rate
...
* @retval None
*/
/* Custom Code - Dante: Uncomment func below for original functionality */
